...
// Prevents Jackson from trying to set fields with no setter (eg calculated properties)
...
public abstract class BaseObject<T extends Annotation> implements Serializable {
	...
	public boolean isNew() {
		return areBusinessKeysEmpty();
	}

	@Transient
	@JsonIgnore
	public boolean areBusinessKeysEmpty() {
		...
		for (AccessibleObject prop : getBusinessKey()) {
			try {
				...
				if (businessKeyValue != null) {
					if (Collection.class.isAssignableFrom(businessKeyValue.getClass())) {
						if ( !((Collection<?>) businessKeyValue).isEmpty() ) {
							return false;
						}
					}
					else {
